 ...
 *   - No id  → Create mode: POST /api/shipments. The tracking
 *             code is generated by the backend during creation
 *             (has to be, to guarantee it's actually unique) —
 *             there's no more separate "preview a code" step.
 *   - id=X   → Edit mode: GET /api/shipments/:id to load real
 *             data + real history, PUT /api/shipments/:id to
 *             save changes (which appends a new history entry
 *             server-side, per the original plan).
 */
